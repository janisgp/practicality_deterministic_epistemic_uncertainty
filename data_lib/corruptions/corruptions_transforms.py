...
gaussian = skf.gaussian
scizoom = scipy.ndimage.zoom
map_coordinates = scipy.ndimage.interpolation.map_coordinates
# njit = numba.njit
# prange = numba.prange
...
frost_imgs = [
  "/cns/lj-d/home/postels/data/frost_imgs/frost1.png",
  "/cns/lj-d/home/postels/data/frost_imgs/frost2.png",
  "/cns/lj-d/home/postels/data/frost_imgs/frost3.png",
  "/cns/lj-d/home/postels/data/frost_imgs/frost4.jpg",
  "/cns/lj-d/home/postels/data/frost_imgs/frost5.jpg",
  "/cns/lj-d/home/postels/data/frost_imgs/frost6.jpg",
]
...
def clipped_zoom(img, zoom_factor):
  ...
  img = scizoom(
      img[top0:top0 + ch0, top1:top1 + ch1], (zoom_factor, zoom_factor, 1),
      order=1)
...
def fog(x, severity=1):
  ...
  if len(x_shape) < 3 or x_shape[2] < 3:
    x += c[0] * plasma_fractal(
        mapsize=map_size, wibbledecay=c[1])[:shape[0], :shape[1]]
  else:
    x += c[0] * \
...
def frost(x, severity=1):
  ...
  filename = [
      resource_filename(frost_imgs[0]),
      resource_filename(frost_imgs[1]),
      resource_filename(frost_imgs[2]),
      resource_filename(frost_imgs[3]),
      resource_filename(frost_imgs[4]),
      resource_filename(frost_imgs[5])
  ][idx]
  ...
  x_start, y_start = np.random.randint(
      0, frost_rescaled.shape[0] - x_shape[0]), np.random.randint(
          0, frost_rescaled.shape[1] - x_shape[1])
...
def snow(x, severity=1):
  ...
  if len(x.shape) < 3 or x.shape[2] < 3:
    x = c[6] * x + (1 - c[6]) * np.maximum(
        x,
        x.reshape(x.shape[0], x.shape[1]) * 1.5 + 0.5)
    snow_layer = snow_layer.squeeze(-1)
  else:
    x = c[6] * x + (1 - c[6]) * np.maximum(
        x,
        cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(x.shape[0], x.shape[1], 1) *
        1.5 + 0.5)
  try:
...
def pixelate(x, severity=1):
  ...
  x = cv2.resize(x, dsize=(int(x_shape[1] * c), int(x_shape[0] * c)),
                 interpolation=cv2.INTER_CUBIC)

  x = cv2.resize(x, dsize=(int(x_shape[1]), int(x_shape[0])),
...
def elastic_transform(image, severity=1):
  ...
  alpha = [250 * 0.05, 250 * 0.065, 250 * 0.085, 250 * 0.1,
           250 * 0.12][severity - 1]
  ...
  dx = (gaussian(
      ...
      sigma,
      mode='reflect',
      truncate=3) * alpha).astype(np.float32)
  dy = (gaussian(
